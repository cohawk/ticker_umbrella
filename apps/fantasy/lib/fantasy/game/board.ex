defmodule Fantasy.Game.Board do
  @moduledoc """
  Game board
  """
  alias Fantasy.{Ship}
  ...
  def create(player_id) do
    ...
    Agent.start(
      fn -> %__MODULE__{player_id: player_id, grid: grid, ships: ships} end,
      name: ref(player_id)
    )
  end
  ...
  """
  def add_ship(_player_id, %Ship{size: size}) when size not in @ships_sizes,
    do: {:error, "Invalid size"}

  def add_ship(_player_id, %Ship{x: x}) when x > @size - 1 or x < 0,
    do: {:error, "Invalid position"}

  def add_ship(_player_id, %Ship{y: y}) when y > @size - 1 or y < 0,
    do: {:error, "Invalid position"}

  def add_ship(_player_id, %Ship{orientation: orientation}) when orientation not in @orientations,
    do: {:error, "Invalid orientation"}
  ...
  def get_opponents_data(player_id) do
    ...
    new_grid =
      board
      |> Map.get(:grid)
      |> Enum.reduce(%{}, fn {coords, value}, acc ->
        Map.put(acc, coords, opponent_grid_value(value))
      end)
  ...
  defp add_ship_to_grid(%__MODULE__{ships: ships} = board, ship) do
    ...
    ships =
      List.update_at(ships, ship_index, &%{&1 | x: ship.x, y: ship.y, coordinates: coordinates})
  ...
  defp set_hit_points(board),
    do: %{board | hit_points: Enum.reduce(board.ships, 0, &(&1.size + &2))}
