...
defmodule Ticker.TimeFrame do
  ...
  def start_link(name, {interval, frame_count, next_intervals}) do
    Logger.info("Starting TimeFrame: #{interval} mins")

    GenServer.start_link(
      __MODULE__,
      {:ok, {name, interval, frame_count, next_intervals}},
      name: via_tuple({name, interval})
    )
  end
  ...
  def handle_cast({:rollup_quotes, quotes}, state) do
    ...
    {:noreply,
     %{
       ...
       :next_intervals => state[:next_intervals],
       :frames => [],
       :frame_key => frame_key
     }}
  end
  ...
  def handle_cast({:rollup_frames, frames}, state) do
    ...
    {remaining_frames, frame_key} =
      cond do
        ...
        true ->
          {current_frames, state[:frame_key]}
      end
  ...
  defp process(frame, frame_key, state) do
    ...
    if state[:next_intervals] do
      Enum.each(state[:next_intervals], fn i ->
        Ticker.TimeFrame.rollup_frames(state[:name], i, [frame])
      end)
    end
...
defmodule Ticker.TimeFrame.Supervisor do
  ...
  def start_link(name) do
    Logger.info("Starting TimeFrame Supervisor: #{name}")
